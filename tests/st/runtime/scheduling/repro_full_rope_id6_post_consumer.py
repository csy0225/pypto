...
class FullRopeTrueCacheId6:
    ...
    def chip_orch(
        ...
        # Post-rope consumer: mimic fa_fused-style read of all_q_padded into a
        # scratch tensor. This is what real e2e has but our repro lacked.
        scratch = pl.create_tensor([BATCH * Q_HEAD_PAD, HEAD_DIM], dtype=pl.BF16)
        for _ in pl.spmd(1, name_hint="post_rope_consume_q"):
            blk = pl.slice(all_q_padded, [BATCH * Q_HEAD_PAD, HEAD_DIM], [0, 0])
            scratch = pl.assemble(scratch, blk, [0, 0])
        return all_q_padded
...
def main():
    work_dir = Path('/tmp/p15_min_rope_post_consumer')
    if work_dir.exists():
