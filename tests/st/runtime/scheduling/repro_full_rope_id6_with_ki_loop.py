...
class FullRopeTrueCacheId6:
    ...
    def chip_orch(
        ...
        for b in pl.parallel(BATCH):
            ...
            cos_lo = pl.slice(rope_cos, [1, HALF], [pos, 0])
            cos_hi = pl.slice(rope_cos, [1, HALF], [pos, HALF])
            sin_lo = pl.slice(rope_sin, [1, HALF], [pos, 0])
            sin_hi = pl.slice(rope_sin, [1, HALF], [pos, HALF])
            with pl.at(level=pl.Level.CORE_GROUP, name_hint="full_rope_kv_cache"):
                for ki in pl.range(1):  # KV_HEADS_LOCAL=1; mirrors real attention_full structure
                    kv_col = ki * HEAD_DIM
                    cache_row = layer_cache_base + (slot_block * 1 + ki) * BLOCK_SIZE + slot_offset
                    k_lo = pl.slice(k_proj_norm, [1, HALF], [b, kv_col])
                    k_hi = pl.slice(k_proj_norm, [1, HALF], [b, kv_col + HALF])
                    ...
                    k_cache = pl.assemble(k_cache, pl.cast(pl.slice(k_proj_norm, [1, HEAD_DIM], [b, kv_col]), target_type=pl.BF16), [cache_row, 0])
                    ...
                    v_cache = pl.assemble(v_cache, pl.cast(pl.slice(v_proj, [1, HEAD_DIM], [b, kv_col]), target_type=pl.BF16), [cache_row, 0])
                    q_block = pl.reshape(pl.slice(q_proj_norm, [1, Q_HEAD_BATCH * HEAD_DIM], [b, ki * Q_HEAD_BATCH * HEAD_DIM]), [Q_HEAD_BATCH, HEAD_DIM])
                    ...
                    base = b * 1 * Q_HEAD_PAD + ki * Q_HEAD_PAD
...
def main():
    work_dir = Path('/tmp/p15_min_rope_with_ki_loop')
    if work_dir.exists():
